...
class Print_and_Delete_inspection:
    ...
    def enter_input_to_delete(self,text):
        locators = [
            (By.XPATH, "//div[contains(@class, 'delete_confirmDeleteBody')]//input[@type='text']"),
            (By.XPATH, "//p[contains(text(), 'Please type DELETE')]/following-sibling::input"),
            (By.XPATH, "//div[contains(text(),'Please type DELETE')]/following::input[1]"),
            (By.XPATH, "//input[@type='text' and ancestor::div[contains(@class,'delete_confirmDeleteBody')]]"),]
        try:
            self.find_element_with_fallback(locators).click()
            self.find_element_with_fallback(locators).clear()
            self.find_element_with_fallback(locators).send_keys(text)
        except Exception as e:
            print('failed to click on delete input field')

    def click_on_confirm_delete(self):
        locators = [
            (By.XPATH, "//button[.//span[normalize-space(text())='Confirm Delete']]"),
            (By.XPATH, "//div[contains(@class, 'delete_footer')]//button[span[text()='Confirm Delete']]"),
            (By.XPATH, "//span[text()='Confirm Delete']/parent::button"),
            (By.XPATH, "//button[contains(@class, 'button_red') and .//span[text()='Confirm Delete']]")]
        try:
            self.find_element_with_fallback(locators).click()
            self.close_modal_if_present()
        except Exception as e:
            print('failed to click on Confirm Delete')