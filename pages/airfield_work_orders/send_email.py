...
class Send_email:
    ...
    def Email_Recipients(self):
        locators = [(By.XPATH, "//span[text()='Email Recipients']/..//div[@class='undefined undefined css-2b097c-container']")]
        try:
            element = self.find_element_with_fallback(locators)
            element.click()
        except Exception as e:
            print("Email Recipients field not found:", str(e))

    def select_user_and_role(self,text):
        locators = [(By.XPATH,f"//div[@class=' css-26l3qy-menu']//div[text()='{text}']")]
        try:
            element = self.find_element_with_fallback(locators)
            element.click()
        except Exception as e:
            print("user or role is not found in selection drop down:", str(e))

    def External_Emails(self,text):
        locators = [(By.XPATH,"//span[text()='External Emails']/..//div")]
        try:
            element = self.find_element_with_fallback(locators)
            element.click()
            element.send_keys(text)
        except Exception as e:
            print("failed to click on external emailss:", str(e))

    def click_on_send_button(self):
        locators = [
            (By.XPATH, "//button[span[text()='Send']]"),
            (By.CSS_SELECTOR, "button.button_button__H5057.button_secondary__1yQBY.emailModal_saveBtn__3h3_S"),
            (By.XPATH, "//button[@type='button' and @tabindex='0']"),]
        try:
            element = self.find_element_with_fallback(locators)
            element.click()
        except Exception as e:
            print('failed to click on send button')
